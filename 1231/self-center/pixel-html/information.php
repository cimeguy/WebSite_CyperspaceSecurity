...
// 	}
	if(empty($_POST['name'])){
		skip_message('information.php', '姓名不得为空');
	}
	if(mb_strlen($_POST['name'],"utf-8")>32){
		
		skip_message('information.php', '用户名长度不要超过32个字符！');
	}
	
	if(mb_strlen($_POST['pw'])<6&&mb_strlen($_POST['pw'])<0){
		skip_message('information.php', '密码不得少于6位！');
	}
	if(mb_strlen($_POST['college'],"utf-8")>32){
		skip_message('information.php',  '学院长度不要超过32个字符！');
	}
	if(mb_strlen($_POST['class'],"utf-8")>16){
			skip_message('information.php', '班级长度不要超过16个字符！');
	}
	
	$_POST=escape($link,$_POST);
    if(mb_strlen($_POST['pw'])>0){      //修改了密码
        $query="update Student set name='{$_POST['name']}',pw=md5('{$_POST['pw']}'),college='{$_POST['college']}',class='{$_POST['class']}' where sno='{$me}'";
        execute($link,$query);
        if(mysqli_affected_rows($link)==1){
    // 		setcookie('netsec[sno]',$_POST['sno']);
            setcookie('netsec[pw]',sha1(md5($_POST['pw'])));
            
            skip_message('information.php', '更改信息成功！');
        }else{
            skip_message('information.php', '未更改任何信息！');
        }
    }
    else{
        $query="update Student set name='{$_POST['name']}',college='{$_POST['college']}',class='{$_POST['class']}' where sno='{$me}'";
        execute($link,$query);
        if(mysqli_affected_rows($link)==1){
...
?>
								<div class="form-group">
                                    <label class="col-md-12">学号（一经注册，不得修改！）</label>
                                    
                                    	<p class="col-md-12"><?php echo $row['sno']?></p>              
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-md-12">
                                        <input type="text" name="class" value="<?php echo $row['class']?>" class="form-control form-control-line"> </div>
                                    </div>
                                </div>
